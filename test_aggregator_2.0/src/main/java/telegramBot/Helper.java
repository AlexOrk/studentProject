...
public class Helper {
    ...
    private final File file = new File("/app/src/main/resources/appData/verification.properties");
    // Verification
    ...
    public synchronized String getHoba(long chat_id) {
        File file = new File("/app/src/main/resources/botData/hoba");
    ...
    public synchronized File fromHtmlToPdf(File htmlFileName) {
        try {
            File file = new File("/app/src/main/java/view/job_list.pdf");
            HtmlConverter.convertToPdf(htmlFileName, file);
            return file;
        } catch (IOException e) {
            e.printStackTrace();
